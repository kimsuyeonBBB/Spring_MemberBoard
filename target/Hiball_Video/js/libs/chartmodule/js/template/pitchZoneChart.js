 ...
 */
define([
        'jquery','underscore','backbone','common', 'statGrade', 'calcstat',
        'text!../../html/template/pitchZoneChart.html',
        'jqueryNum', 'jqueryCanvas'
], function($, _, Backbone, Common, StatGrade, CalcStat, PitchingZoneChart) {
	var model = Backbone.Model;
	var chartType = "spreadPoint";
	var chartName = "";
	var zoneClickEventHandler = null;
	var pitchZoneEventHandler = null;
	var chartSize = 0;
	var battingHand = "R";
	
	var chartInitialHandler = function(viewObj, name, size, bHand, eventHandler, clickEventHandler, pitchEventHandler){
		if(name!=null && name!="") {
			chartName = name;
		} else {
			chartName = "pit";
		...
		if(typeof pitchEventHandler != "undefined"&& pitchEventHandler!=null){
			pitchZoneEventHandler = pitchEventHandler;
		}
		this.$el = viewObj;
	}
...
//			pinZoneHandler(pinZoneOnOff);
	...
	function chartHandler(data) {
		...
		if(typeof classification == "undefined"){
			...
			caledData = calcRateForZone(data, "pitched");
		} else{
			caledData = calcRateForZone(data, classification);
		} 
		
		drawZoneChart(caledData);
...
//	}
	...
	var calcRateForZone = function (data, classification) {
		var ratingDataForZone = CalcStat.calcRateForZone(data, classification);
		return ratingDataForZone;
	}
	...
	var drawZoneChart = function(data) {
		...
		_.each(pitchZone, function(item){
			...
			if(gradeType == "judge"){
				gradeObj = StatGrade.sbJudgedRateGrade(zoneData, className);
			}else{
				gradeObj = StatGrade.gradeFor5by5Grid(zoneData);
			}
