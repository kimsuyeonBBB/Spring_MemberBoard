define([
	...
	function chartHandler(data) {
		...
		if (chartType=="spreadChart") {
			drawSpreadChart(data);
			$("#forZoneLegend").css("display", "none");
		} else if (chartType=="rateChart") {
			drawZoneChart(data);
...
//				} else if (gradeType=="hr"){
//					gradeObj = StatGrade.hrRateGrade(zoneData);
//				} else if (gradeType=="fo"){
//					gradeObj = StatGrade.flyOutRateGrade(zoneData);
//				} else if (gradeType=="go"){
//					gradeObj = StatGrade.groundOutRateGrade(zoneData);
//				} else if (gradeType=="standing"){
...
//			}
			...
			if(gradeType == "judge") {
				if(typeof zoneData != "undefined"){
					gradeObj = StatGrade.catcherZonePitchingBallRate(zoneData.zonePitchingBallRate);
				}else{
					gradeObj["value"] = 0;
				}
			}else{
	...
	var drawSpreadChart = function(data) {
		...
		_.each(data, function(item){
			x = item.pitchZoneCodePositionX;
			y = item.pitchZoneCodePositionY;
